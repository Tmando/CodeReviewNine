...
<body>
  ...
  <?php
    ...
    while(!is_null($row)){
      ...
      echo createForm($authorid,$author);
      echo "</div>";
      echo "</div>";
      $row=mysqli_fetch_array($res, MYSQLI_ASSOC);
    }

    function createForm($authorid,$author){
    $tag = "<form action=\"authorBookList.php\" method=\"post\">";
    $tag .= "<button class=\"btn btn-link\" type=\"submit\" name=\"authorid\" value=\"".$authorid."\">".$author."</button>";
    $tag .= "<input style=\"display:none\" name=\"fromAuthor\" value=\"true\">";
    $tag .= "</form>";
